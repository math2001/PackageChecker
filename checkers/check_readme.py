...
class CheckReadme(Checker):
    ...
    def run(self):
        ...
        if not CheckReadme.HAS_MARKDOWN_IMAGE.search(self.get_file_content('README.md')):
            if 'color scheme' in self.labels or 'theme' in self.labels \
                or self.glob_files(extension='.sublime-theme') \
                or self.glob_files(extension='.tmTheme'):
                return self.fail("Because your package is a theme or a color scheme, it'd be "
            ...
            self.warn("It'd be great if you'd add a demo (an image) in your README.",
                      "The valid format for an image in Markdown is "
                      "![alt text](your/image/path). \nSee {} "
                      "for more infos".format(MARKDOWN_CHEAT_SHEET))
